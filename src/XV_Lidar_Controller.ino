/*
  ...
  The F() macro in the Serial statements tells the compiler to keep your strings
  in PROGMEM
*/
...
const int SHOW_ALL_ANGLES =
    N_ANGLES; // value means 'display all angle data, 0..359'

struct EEPROM_Config {
  byte id;
  char version[6];
  int motor_pwm_pin;   // pin connected to mosfet for motor speed control
  double rpm_setpoint; // desired RPM (uses double to be compatible with PID
                       // library)
  double rpm_min;
  double rpm_max;
...
unsigned int rpm_err_thresh = 10; // 2 seconds (10 * 200ms) to shutdown motor
                                  // with improper RPM and high voltage
...
const int N_ELEMENTS_PER_QUAD =
    4; // viz., 0=distance LSB; 1=distance MSB; 2=sig LSB; 3=sig MSB
...
const int OFFSET_TO_CRC_L =
    OFFSET_TO_4_DATA_READINGS + (N_DATA_QUADS * N_ELEMENTS_PER_QUAD);
...
const byte INVALID_DATA_FLAG =
    (1 << 7); // Mask for byte 1 of each data quad "Invalid data"

/* REF: https://github.com/Xevel/NXV11/wiki
  The bit 7 of byte 1 seems to indicate that the distance could not be
  calculated. It's interesting to see that when this bit is set, the second byte
  is always 80, and the values of the first byte seem to be only 02, 03, 21, 25,
  35 or 50... When it's 21, then the whole block is 21 80 XX XX, but for all the
  other values it's the data block is YY 80 00 00 maybe it's a code to say what
  type of error ? (35 is preponderant, 21 seems to be when the beam is
  interrupted by the supports of the cover) .
*/
const byte STRENGTH_WARNING_FLAG =
    (1 << 6); // Mask for byte 1 of each data quat "Strength Warning"

/*
  The bit 6 of byte 1 is a warning when the reported strength is greatly
  inferior to what is expected at this distance. This may happen when the
  material has a low reflectance (black material...), or when the dot does not
  have the expected size or shape (porous material, transparent fabric, grid,
...
uint16_t aryDist[N_DATA_QUADS] = {
    0, 0, 0, 0}; // thre are (4) distances, one for each data quad
// so the maximum distance is 16383 mm (0x3FFF)
uint16_t aryQuality[N_DATA_QUADS] = {0, 0, 0, 0}; // same with 'quality'
uint16_t motor_rph = 0;
uint16_t startingAngle = 0; // the first scan angle (of group of 4, based on
                            // 'index'), in degrees (0..359)

// non-zero == INVALID_DATA_FLAG or STRENGTH_WARNING_FLAG is set
byte aryInvalidDataFlag[N_DATA_QUADS] = {0, 0, 0, 0};
...
void loop() {
  ...
  if (Serial1.available() > 0) {
    ...
    if (eState == eState_Build_Packet) {
      // State 2: Build a complete data packet
      Packet[ixPacket++] = inByte;
...
void resetState() {
  ...
  for (ixPacket = 0; ixPacket < PACKET_LENGTH;
       ixPacket++) // clear out this packet
    Packet[ixPacket] = 0;

  // This packet is done -- look for next COMMAND byte
  ixPacket = 0;
  ...
  if (xv_config.show_dist) {
    ...
    for (int ix = 0; ix < N_DATA_QUADS; ix++) {
      if (!xv_config.aryAngles[startingAngle + ix])
        // if we're not supposed to display that angle
        continue;

      if (aryInvalidDataFlag[ix] & BAD_DATA_MASK) {
        // if LIDAR reported a data error...

        if (xv_config.show_errors) {
          // if we're supposed to show data errors...
          Serial.print(F("A,"));
...
/*
   ...
   index is the index byte in the 90 packets, going from A0 (packet 0, readings
   0 to 3) to F9 (packet 89, readings 356 to 359). Enter with: N/A Uses:
   Packet ledState gets toggled if angle = 0 ledPin = which pin the LED is
   connected to ledState = LED on or off xv_config.show_dist = true if we're
   supposed to show distance curMillis = milliseconds, now lastMillis =
   milliseconds, last time through this subroutine xv_config.show_interval =
   true ==> display time interval once per revolution, at angle 0 Calls:
   digitalWrite() - used to toggle LED pin Serial.print Returns:    The first
   angle (of 4) in the current 'index' group
*/
uint16_t processIndex() {
  ...
  if (angle == 0) {
    ...
    if (xv_config.show_interval) {
      Serial.print(
          F("T,")); // Time Interval in ms since last complete revolution
...
/*
   ...
   speed is two-bytes of information, little-endian. It represents the speed, in
   64th of RPM (aka value in RPM represented in fixed point, with 6 bits used
   for the decimal part). Enter with: N/A Uses:       Packet angle = if 0 then
   enable display of RPM and PWM xv_config.show_rpm = true if we're supposed to
   display RPM and PWM Calls:      Serial.print
*/
void processSpeed() {
...
/*
   Data 0 to Data 3 are the 4 readings. Each one is 4 bytes long, and organized
   as follows : byte 0 : <distance 7:0> byte 1 : <"invalid data" flag>
   <"strength warning" flag> <distance 13:8> byte 2 : <signal strength 7:0> byte
...
/*
   ...
   Uses:       Packet
               dist[] = sets distance to object in binary: ISbb bbbb bbbb bbbb
                                       so maximum distance is 0x3FFF (16383
   decimal) millimeters (mm) Calls:      N/A Exits with: 0 = okay Error:      1
   << 7 = INVALID_DATA_FLAG is set 1 << 6 = STRENGTH_WARNING_FLAG is set
*/
byte processDistance(int iQuad) {
  uint8_t dataL, dataM;
  aryDist[iQuad] = 0; // initialize
  int iOffset = OFFSET_TO_4_DATA_READINGS + (iQuad * N_DATA_QUADS) +
                OFFSET_DATA_DISTANCE_LSB;
  // byte 0 : <distance 7:0> (LSB)
  // byte 1 : <"invalid data" flag> <"strength warning" flag> <distance 13:8>
  // (MSB)
  dataM = Packet[iOffset + 1];    // get MSB of distance data + flags
  if (dataM & BAD_DATA_MASK)      // if either INVALID_DATA_FLAG or
                                  // STRENGTH_WARNING_FLAG is set...
...
void initEEPROM() {
  ...
  xv_config.motor_pwm_pin =
      BOARD_MOTOR_PWM_PIN; // pin connected N-Channel Mosfet
...
/*
   showInterval - enable display of Time interval (which happens once per
   revolution, at angle 0
*/
void showInterval() {
  xv_config.show_interval = true;
  if (!xv_config.show_dist) { // suppress activity message if we're executing
                              // 'show distance'
    Serial.println(F(" "));
...
void hideErrors() { // disable error display
  xv_config.show_errors = false;
  if (!xv_config.show_dist) {
    // suppress activity message if we're executing 'show distance'
    Serial.println(F(" "));
...
void hideRPM() {
  xv_config.show_rpm = false;
  if (xv_config.show_dist ==
      false) { // suppress activity message if we're executing 'show distance'
    Serial.println(F(" "));
...
/*
   ...
   Enter with: N/A
   Uses:       xv_config.aryAngles (an array of 360 booleans) is set to
   appropriate values Calls:      showDist Exits with: N/A TEST THIS STRING:
   SetAngles 16-20, 300-305, 123-124, 10
*/
void setAngle() {
  char c, *arg;
  ...
  do {
    ...
    // validate 'from' and 'to' and set 'xv_config.aryAngles' with correct
    // values
  ...
  if (syntax_error) {
    ...
    Serial.println(F("       In a from-to pair, the 1st value must be lowest. "
                     "From-to pairs can overlap ranges."));
...
void motorCheck() { // Make sure the motor RPMs are good else shut it down
  ...
  if (now - motor_check_timer > motor_check_interval) {
    if ((motor_rpm < xv_config.rpm_min or motor_rpm > xv_config.rpm_max) and
        pwm_val > 1000) {
      rpm_err++;
    } else {
      rpm_err = 0;
...
void help() {
  ...
  Serial.println(
      F("  SaveConfig    - Save the running configuration to EEPROM"));
  Serial.println(F("  ResetConfig   - Restore the original configuration"));
  Serial.println(F("  SetAngle      - Show distance data for a multiple angles "
                   "(Ex: SetAngle 0, 15-30, 45-50, 10)"));
  Serial.println(F(
  ...
  Serial.println(
      F("  ShowRaw       - Enable the output of the raw lidar data (default)"));
  Serial.println(
      F("  HideRaw       - Stop outputting the raw data from the lidar"));
  Serial.println(F("  ShowDist      - Show angles with distance data"));
  Serial.println(F("  HideDist      - Hide the distance data"));
  Serial.println(F("  ShowErrors    - Show all error types (CRC, Signal "
  ...
  Serial.println(
      F("  ShowAll       - Show the distance, errors, RPMs and interval data"));
  Serial.println(
      F("  HideAll       - Hide the distance, errors, RPMs and interval data"));
  ...
  Serial.println(
      F("  SetSampleTime - Set the frequency the PID is calculated (ms)"));
  ...
  Serial.println(
      F("    S = LIDAR reports Poor signal strength for this angle"));
  Serial.println(F(" "));
}
